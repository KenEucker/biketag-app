...
import { useBikeTagClientStore } from 'src/stores/biketagClient';
...
const bikeTagClientStore = useBikeTagClientStore();
...
onMounted(async () => {
  if (route.params.name) {
    await bikeTagClientStore.fetchTagsList(
...
const rows = computed((): QTableProps['rows'] => {
  if (state.searchTag) {
    return bikeTagClientStore.getGamesTag.filter((item) => {
      if (item.name) {
        return (
    ...
    return bikeTagClientStore.getGamesTag;
  }
});
...
onUnmounted(() => {
  bikeTagClientStore.$state.gamesTag = [];
});
</script>
<template>
  <q-card flat bordered class="mb-10">
    ...
    <div class="px-1 md:px-0">
      <q-table
        ...
        :loading="bikeTagClientStore.getGamesLoader"
        :rows="rows"
        :columns="columns"
        row-key="id"
      ...
      >
        <template #header="props">
          <q-tr class="bg-gray-100 text-gray-500 uppercase">
            ...
            <q-th :props="props" key="hint" rowspan="2" class="w-[500px]"
              >Hint</q-th
            >
          </q-tr>
        </template>
        ...
        <template #bottom>
          <div class="md:flex md:items-center md:justify-between w-full">
            <div class="text-left md:!block !hidden">
              Total row count: {{ bikeTagClientStore.getGamesTag.length }}
            </div>
