...
onMounted(async () => {
  // Get game list
  await bikeTagClientStore.fetchGamesList();
});

// data table data using computed
const rows = computed((): QTableProps['rows'] => {
  if (state.searchGame) {
    return bikeTagClientStore.getGames.filter((item) => {
      if (item.name) {
        return (
    ...
    return bikeTagClientStore.getGames;
  }
});
      ...
      <template #body-cell-action="props">
        <q-td :props="props">
          <q-btn
            ...
            @click="
              router.push(
                '/guest/games/' + props.row.name + '/' + props.row.mainhash
              )
            "
          ></q-btn>
        </q-td>
      ...
      <template #bottom>
        <div class="md:flex md:items-center md:justify-between w-full">
          <div class="font-medium text-right md:text-left md:!block !hidden">
            Total row count: {{ bikeTagClientStore.getGames.length }}
          </div>
