  ...
  actions: {
    ...
    async updateGame(game: Game, updatedGame: Game) {
      if (game) {
        ...
        if (
          game.logo != updatedGame.logo ||
          game.subreddit != updatedGame.subreddit
        ) {
    ...
    async launchGame(game: Game, tag: Tag) {
      ...
      const biketagAdmin = new BikeTagClient(adminConfig)
      const launchGameResult = await biketagAdmin.updateGame(game, {
        source: 'sanity'
      })
      return [launchGameResult, await biketagAdmin.updateTag(tag)]
    },
