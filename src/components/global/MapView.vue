...
function getCenter() {
  return props.center // Set initial map center
}

function getGps() {
  return props.gps // Set initial map center
}
...
<template>
  <GMapMap
    :center="!getGps().lat && !getGps().lng ? getCenter() : getGps()"
    :zoom="!getGps().lat && !getGps().lng ? 3 : 14"
    map-type-id="roadmap"
  >
    <GMapMarker
      :icon="pinIcon"
      ...
      :draggable="true"
      :clickeable="true"
      @dragend="(e: DragType) => emit('dragend', clean(e))"
    />
  </GMapMap>
</template>
