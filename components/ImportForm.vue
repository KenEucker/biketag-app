  ...
  <form class="export-form--centered">
    <div class="export-form--centered__cnt">
      <input
        ...
        @change="loadFile"
      />
      <u-button class="justify-content-between" fill="clear" @click.prevent="getFile">
        <Icon name="ic:outline-round-cloud-download"/>
        <label class="margin-start"> {{ props.text }} </label>
      </u-button>
    </div>
  </form>
</template>
